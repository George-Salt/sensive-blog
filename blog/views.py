...
def index(request):
    ...
    most_popular_posts = posts.order_by('-num_likes')[:5]

    most_fresh_posts = list(Post.objects.order_by('published_at'))[-5:]
...
def post_detail(request, slug):
    author = Post.objects.select_related('author')
    post = author.get(slug=slug)
